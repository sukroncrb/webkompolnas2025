...
/** source: /home/sukroncrb2025/Project/kompolnas/website/vendor/abiesoft/Http/../../../templates/admin/form/ppid.latte */
final class Template_964bcd439f extends Latte\Runtime\Template
{
	public const Source = '/home/sukroncrb2025/Project/kompolnas/website/vendor/abiesoft/Http/../../../templates/admin/form/ppid.latte';
...
';
	...
	<div class="container-fluid">
		<div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
			<div class="card-body px-4 py-3">
				<div class="row align-items-center">
					<div class="col-9">
						<h4 class="fw-semibold mb-8">Semua Perngaduan</h4>
		...
		echo '">Home</a>
								...
								<li class="breadcrumb-item" aria-current="page">PPID</li>
							</ol>
						</nav>
					</div>
					<div class="col-3">
				...
				<div class="row">
					<div class="col-md-4 col-xl-3">
						<form class="position-relative" id=\'formSearch\' name=\'formSearch\' onSubmit=\'return searchData()\'>
                            ...
                            <input type=\'hidden\' id=\'tabel\' name=\'tabel\' value=\'ppid\'>
...
async function getDataFromDB() {
    ...
    const url = baseurl + "api/form/ppid";
    try {
...
function getLoadData(x) {
    ...
    setTimeout(()=>{
        for(let i=0; i< semuadata.length; i++){
            let statusppid = `<span class=\'badge text-bg-danger\'>Belum ditangani</span>`;
            ...
            if(i == (startfrom + 1) || i < (startfrom + jumlahitem)){
                itemdata += `
                  <tr>
                    ...
                    <td>
                      <div class=\'text-center\'>`+semuadata[i].nama+`</div>
                    </td>
                    <td>
                      <div class=\'text-center\'>`+semuadata[i].dibuat+`</div>
                    </td>
                    <td>
                      <div class=\'text-center\'>`+statusppid+`</div>
                    </td>
                    <td>
                      <div class="action-btn text-center" style=\'display: flex; justify-content: center; align-items: center; gap: 5px;\'>
                        <a href="`+baseurl+prefix+`/ppid/`+semuadata[i].id+`/read" class="text-dark hover:text-primary edit ms-2">
                          <i class="ti ti-eye fs-5"></i>
                        </a>
                        <form method="post" data-id="`+semuadata[i].id+`" data-label="`+semuadata[i].nomor+`" id="formHapus`+semuadata[i].id+`" name="formHapus`+semuadata[i].id+`" onSubmit="return hps([this.dataset.id,this.dataset.label])">
...
function hps(x) {
  Swal.fire({
    title: "Konfirmasi",
    html: "Ingin menghapus permohonan <strong class=\'text-danger\'>"+x[1] +"</strong> ?",
    icon: "warning",
...
function doRemove(x) {
    ...
    fetch(baseurl + \'api/form\', {
        ...
        if(result.code == "200"){
          Swal.fire({
            title: "Ok!",
            html: "Permohonan <strong class=\'text-danger\'>"+result.data.nomor +"</strong> sudah dihapus.",
            icon: "success"
          });
